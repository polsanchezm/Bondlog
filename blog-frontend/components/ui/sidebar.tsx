...
// import { Message } from "@/components/ui/chat/hooks/data";
...
export function Sidebar({ chats, isCollapsed, isMobile }: SidebarProps) {
  return (
    <div className="relative flex flex-col h-full bg-gray-100 dark:bg-gray-900 gap-4 p-2">
      {!isCollapsed && (
        <div className="flex justify-between p-2 items-center">
          <div className="flex gap-2 items-center text-2xl">
            <p className="font-medium">Chats</p>
      ...
      <nav className="grid gap-1 px-2">
        {chats.map((chat, index) => (
          <button
            ...
            className={cn(
              "flex items-center gap-4 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-all",
              chat.variant === "secondary" && "bg-gray-200 dark:bg-gray-700"
            )}
          >
            <img
              src={chat.avatar}
            ...
            {!isCollapsed && (
              <div className="flex flex-col">
                <span className="text-start">{chat.username}</span>
                {chat.messages.length > 0 && (
                  <span className="text-xs text-gray-500 w-full line-clamp-2 text-start">
